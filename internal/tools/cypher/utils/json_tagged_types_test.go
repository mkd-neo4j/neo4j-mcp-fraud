	...
	tests := []struct {
		...
		},
		{
			...
			arguments: map[string]any{
				...
				"params": utils.Params{"list": []any{1, 2.0, 3.5}},
			},
			wantQuery:  "MATCH (n) WHERE n.list IN $list RETURN n",
			wantParams: utils.Params{"list": []any{int64(1), int64(2), float64(3.5)}},
		},
		{
		...
		},
		{
			...
			arguments: map[string]any{
				...
				"params": utils.Params{"balance": -100, "adjustment": -1.5},
			},
			wantQuery:  "MATCH (n) WHERE n.balance = $balance AND n.adjustment = $adjustment RETURN n",
			wantParams: utils.Params{"balance": int64(-100), "adjustment": float64(-1.5)},
		},
	}
...
func TestBindArgumentsWithWriteCypherInput(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			var args write.WriteCypherInput
			err := request.BindArguments(&args)
...
func TestConvertNumbers(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := utils.ConvertNumbers(tt.input)
