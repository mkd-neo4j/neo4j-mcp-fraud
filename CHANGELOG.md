...
## v2.0.0 - 2026-01-14
### Major
* **Rebranded to Neo4j Fraud MCP**: The project has been rebranded from "Neo4j MCP" to "Neo4j Fraud MCP" to reflect its specialized focus on fraud detection and financial crime investigation.
* **Repository Moved**: New repository location at `github.com/mkd-neo4j/neo4j-mcp-fraud`
* **Binary Renamed**: The binary is now called `neo4j-fraud-mcp` (previously `neo4j-mcp`)
* **Module Path Updated**: Go module path changed to `github.com/mkd-neo4j/neo4j-mcp-fraud`
* **Application Name**: Internal application name updated to `FRAUD-MCP4NEO4J` for telemetry and tracking
* **Fraud Detection Tools**: Added specialized fraud detection tools including synthetic identity detection
* **Enhanced Documentation**: All documentation updated to emphasize fraud detection use cases, including fraud-specific examples and guidelines

### Breaking Changes
* Binary name changed from `neo4j-mcp` to `neo4j-fraud-mcp` - update all scripts and configurations
* Module import path changed - Go code importing this module must update imports
* Configuration file keys remain the same, but server name in client configs should be updated
